...
const GameController: React.FC<GameControllerProps> = ({
  ...
  useEffect(() => {
    const initializeGame = async () => {
      try {
        console.log("[DEBUG] GameController - Initializing game");
        setLoading(true);

        // Run character migration to fix experience persistence issues
        console.log(
          "[DEBUG] GameController - Running character migration to fix experience persistence"
        );
        const migration = new CharacterMigration();
        await migration.fixExperiencePersistence();

        // Check localStorage directly to see what characters exist
        console.log(
          "[DEBUG] GameController - Checking localStorage for existing characters"
        );
        LocalStorageDebugger.listAllCharacters();
        console.log("[DEBUG] GameController - localStorage check complete");

        const characterService =
          getContainer().resolve<ICharacterService>("CharacterService");
        const worldMapService =
          getContainer().resolve<IWorldMapService>("WorldMapService");
        ...
        // Create default character if not provided and no characters exist
        if (!charId && allCharacters.length === 0) {
          console.log(
            "[DEBUG] GameController - No characters found, creating default character"
          );
          const defaultCharacter = await characterService.createCharacter(
            "Hero"
          );
          charId = defaultCharacter.getId();
          setCharacterId(charId);
          console.log(
            "[DEBUG] GameController - Created default character with ID:",
            charId
          );
        } else if (!charId && allCharacters.length > 0) {
          // Use the first existing character if no specific ID was provided
          charId = allCharacters[0].getId();
          setCharacterId(charId);
          console.log(
            "[DEBUG] GameController - Using existing character with ID:",
            charId
          );

          // Log the character stats
          const stats = allCharacters[0].getStats();
          console.log("[DEBUG] GameController - Existing character stats:", {
            id: charId,
            name: allCharacters[0].getName(),
            level: stats.level,
          ...
          console.log(
            "[DEBUG] GameController - Loading existing character:",
            charId
          );
          if (charId === null) {
            throw new Error(
          ...
          if (existingCharacter) {
            ...
            console.log("[DEBUG] GameController - Loaded character stats:", {
              id: existingCharacter.getId(),
            ...
            // Create a new character as fallback
            console.log("[DEBUG] GameController - Creating fallback character");
            const fallbackCharacter = await characterService.createCharacter(
              "Hero"
            );
            charId = fallbackCharacter.getId();
            setCharacterId(charId);
            console.log(
              "[DEBUG] GameController - Created fallback character with ID:",
              charId
            );
          }
        }
        ...
        if (!mapId) {
          ...
          const monsterService =
            getContainer().resolve<IMonsterService>("MonsterService");

          // Create monsters if they don't exist yet
          const createMonsterIfNotExists = async (
            id: string,
            name: string,
            type: string,
            health: number,
            ...
            if (!existingMonster) {
              ...
              const monsterRepository =
                getContainer().resolve<MonsterRepository>("MonsterRepository");
          ...
          await createMonsterIfNotExists(
            "wolf",
            "Wolf",
            "beast",
            20,
  ...
  useEffect(() => {
    const spawnEnemies = async () => {
      ...
      try {
        const enemyService =
          getContainer().resolve<IEnemyService>("EnemyService");
        const monsterService =
          getContainer().resolve<IMonsterService>("MonsterService");
        ...
        if (monsters.length > 0 && worldMapId) {
          console.log("[DEBUG] Spawning enemies on the map");

          // Spawn a few enemies on the map
          for (let i = 0; i < 5; i++) {
            const randomMonsterIndex = Math.floor(
              Math.random() * monsters.length
            );
            const monsterId = monsters[randomMonsterIndex].getId();

            // Random position on the map
            const x = Math.floor(Math.random() * 100);
            const y = Math.floor(Math.random() * 100);

            await enemyService.createEnemy(
              monsterId,
              x,
              y,
              worldMapId as string
            );
          }

          setEnemiesSpawned(true);
          console.log("[DEBUG] Enemies spawned successfully");
        }
      } catch (error) {
        console.error("[DEBUG] Error spawning enemies:", error);
      }
    };
  ...
  const handleBattleEnd = async (result: BattleResult) => {
    try {
      ...
      if (result.victory) {
        console.log("[DEBUG] Battle victory processing started");
        console.log("[DEBUG] Battle result:", {
          victory: result.victory,
          experienceGained: result.experienceGained,
          itemDropped: result.itemDropped?.getName(),
          monsterRecruited: result.monsterRecruited?.getName(),
        });
        ...
        console.log(
          `[DEBUG] Getting character ${characterId} before adding experience`
        );
        const characterBefore = await characterService.getCharacter(
          characterId!
        );

        if (!characterBefore) {
          console.error(
            `[DEBUG] Character ${characterId} not found before adding experience`
          );
          throw new Error(`Character ${characterId} not found`);
        }

        const beforeStats = characterBefore.getStats();
        const previousLevel = beforeStats.level || 1;
        console.log("[DEBUG] Character before adding experience:", {
          id: characterId,
          level: previousLevel,
        ...
        console.log(
          `[DEBUG] Adding ${result.experienceGained} experience to character ${characterId}`
        );
        ...
        // Verify the character was updated by getting it again
        console.log(
          `[DEBUG] Getting character ${characterId} after adding experience`
        );
        const updatedCharacter = await characterService.getCharacter(
          characterId!
        );

        if (!updatedCharacter) {
          console.error(
            `[DEBUG] Character ${characterId} not found after adding experience`
          );
          throw new Error(`Character ${characterId} not found after update`);
        }

        const afterStats = updatedCharacter.getStats();
        const currentLevel = afterStats.level || 1;
        console.log("[DEBUG] Character after adding experience:", {
          id: characterId,
          level: currentLevel,
        ...
        console.log("[DEBUG] Testing localStorage functionality directly");
        LocalStorageDebugger.testLocalStorage();

        // Inspect the character data in localStorage
        LocalStorageDebugger.inspectCharacter(characterId!);

        // List all characters in localStorage
        LocalStorageDebugger.listAllCharacters();

        // Run comprehensive character persistence test
        console.log("[DEBUG] Running comprehensive character persistence test");
        CharacterPersistenceTest.runTest().catch((err) => {
          console.error(
            "[DEBUG] Error running character persistence test:",
            err
          );
        });

        // Run character migration to ensure experience is properly saved
        console.log(
          "[DEBUG] Running character migration to fix experience persistence"
        );
        const migration = new CharacterMigration();
        await migration.fixExperiencePersistence().catch((err) => {
          console.error(
            "[DEBUG] Error running experience persistence fix:",
            err
          );
        });

        // Force reload the character to ensure we have the latest data
        console.log("[DEBUG] Force reloading character to ensure latest data");
        const reloadedCharacter = await characterService.getCharacter(
          characterId!,
          true
        );
        ...
        if (result.itemDropped) {
          ...
          await characterService.addItem(
            characterId!,
            result.itemDropped.getId()
          );
        }

        if (result.monsterRecruited) {
          console.log(
            `Monster recruited: ${result.monsterRecruited.getName()}`
          );
          await characterService.addAlly(
            characterId!,
            result.monsterRecruited.getId()
          );
        }
      ...
      console.log(
        "Character data refreshed after battle:",
        updatedCharacter?.getStats()
      );
    } catch (err) {
  ...
  const handleEnemyBattle = async (enemyId: string) => {
    try {
      const enemyService =
        getContainer().resolve<IEnemyService>("EnemyService");
      const battleService =
        getContainer().resolve<IBattleService>("BattleService");
      ...
      const battleState = await battleService.initiateBattle(
        characterId!,
        enemy.getMonsterId()
      );
