...
export const useBattleActions = ({
  ...
  const handleSkillAction = async (skillId: string) => {
    ...
    if (isProcessing || battleState.isOver) {
      console.log(
        `🚫 [BATTLE] Skill action blocked - isProcessing: ${isProcessing}, isOver: ${battleState.isOver}`
      );
      return;
    }
    ...
    try {
      ...
      const skillResult = skillService.useSkill(
      ...
      console.log(
        `✅ [BATTLE] Skill used successfully: ${skillResult.message}`
      );
      ...
      setTimeout(async () => {
        try {
          const monsterState = await battleService.performMonsterAction(
            updatedState
          );
  ...
  const handleAction = async (action: "attack" | "defend" | "flee") => {
    ...
    console.log(
      `🎯 [BATTLE] Current state - isProcessing: ${isProcessing}, isOver: ${battleState.isOver}`
    );
    console.log(`🎯 [BATTLE] Current turn: ${battleState.turn}`);

    if (isProcessing || battleState.isOver) {
      console.log(
        `🚫 [BATTLE] Action blocked - isProcessing: ${isProcessing}, isOver: ${battleState.isOver}`
      );
      return;
    }
    ...
    try {
      ...
      const updatedState = await battleService.performCharacterAction(
        battleState,
        action
      );
      ...
      if (updatedState.isOver) {
        console.log(
          `🏁 [BATTLE] Battle is over, not proceeding to monster turn`
        );
        // Don't automatically call onBattleEnd here
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          // Track damage taken
          const previousCharacterHealth =
            updatedState.character.getStats().health;
          const currentCharacterHealth =
            monsterState.character.getStats().health;
          const damageToCharacter =
            previousCharacterHealth - currentCharacterHealth;

          if (damageToCharacter > 0) {
            setDamageTaken((prev) => prev + damageToCharacter);
            console.log(
              `💔 [BATTLE] Damage taken by character: ${damageToCharacter}`
            );
          }
          ...
          if (monsterState.isOver && monsterState.result) {
            console.log(
              `🏁 [BATTLE] Battle ended with result:`,
              monsterState.result
            );
            // Don't automatically call onBattleEnd here
            // The battle result screen will be shown and user must click Continue
  ...
  const handleFlee = () => {
    ...
    try {
      ...
      const fleeState: BattleState = {
        ...
        log: [
          ...battleState.log,
          `${battleState.character.getName()} fled from battle!`,
        ],
      ...
      console.log(
        "🏃 [BATTLE] Flee successful. onBattleEnd will be called when user clicks Continue"
      );
